...
import { TextField, FormControl, InputLabel, MenuItem, Select, Button, Container, Typography, ButtonGroup } from '@mui/material';
    ...
    return (
        <Container maxWidth={false} sx={{ mx: 0, overflow: 'hidden' }}>
            <Grid container spacing={3} sx={{ my: 2, height: '100%' }}>
                <Grid item  xl={3} lg={3} md={4} sm={12} xs={12} sx={{ pt: 3 }}>
                    ...
                    <Button onClick={() => setOpen(true)} variant="outlined" fullWidth>Tạo MCP</Button>
                    <DialogMCP open={open} onClose={() => setOpen(false)} />

                    {/* <Typography>MCP hien thi: {displayMCP.map(x => x._id).join(', ')}</Typography>
                    <Typography>Truck hien thi: {displayTruck.map(x => x._id).join(', ')}</Typography> */}

                </Grid>
                <Grid item xl={9} lg={9}  md={8} sm={12} xs={12} >
                    <MapContainer style={{ height: '500px' }} center={[10.876958, 106.794195]} zoom={15} scrollWheelZoom={true}>
                        <Routing />
                        ...
                        {mcps.map(mcp => {
                            ...
                            return (
                                <Marker icon={icon} position={[mcp.x.$numberDecimal, mcp.y.$numberDecimal]} key={mcp._id}>
                                    <Popup>
                                        ...
                                        <ButtonGroup variant="contained" size="xsmall">
                                            ...
                                            <Button color="error" onClick={() => handleDeleteMCP(mcp._id)}><DeleteIcon /></Button>
                                        </ButtonGroup>
                                        <DialogMCP curMCP={mcp} open={open} onClose={() => setOpen(false)} />
                                    </Popup>
                                    <Tooltip direction="bottom" style={{ marginTop: '-20px' }}>MCP ID: {mcp._id}</Tooltip>
                                </Marker>
                            )
                        })}
                        ...
                        {trucks.map(truck => {
                            return (
                                <Marker icon={truckIcon} position={[truck.x.$numberDecimal, truck.y.$numberDecimal]} key={truck._id}>
                                    <Tooltip direction="bottom" style={{ bottom: '0px' }}>Truck ID: {truck._id}</Tooltip>
                                    <Popup>
